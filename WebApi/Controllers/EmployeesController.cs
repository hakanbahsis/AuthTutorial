...
[MustHavePermission(AppFeature.Employees,AppAction.Default)]
public class EmployeesController : MyBaseController<EmployeesController>
{
    ...
    {
        var response=await MediatorSender.Send( new CreateEmployeeCommand { CreateEmployeeRequest=request});
    ...
    {
        var response=await MediatorSender.Send(new UpdateEmployeeCommand { UpdateEmployeeRequest=updateEmployee });
        if (response.IsSuccessful)
            return Ok(response);
        return BadRequest(response);
    }
    ...
    public async Task<IActionResult> DeleteEmployee([FromBody] Guid employeeId)
    {
        var response=await MediatorSender.Send(new DeleteEmployeeCommand { EmplooyeId=employeeId });
        if (response.IsSuccessful)
            return Ok(response);
